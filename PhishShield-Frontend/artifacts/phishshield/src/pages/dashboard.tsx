...
function buildDisplayLabel(score = 0, confidencePercent?: number, trustScore?: number, isSafeZeroRisk = false) {
  ...
  return `Confidence: ${confidence}% • Trust score: ${displayTrustScore}/${trustScale}`;
}
  ...
  return (
    <div className="max-h-96 overflow-y-auto rounded-2xl border border-card-border/70 bg-[linear-gradient(140deg,rgba(15,23,42,0.96),rgba(15,23,42,0.88))] p-4 text-white shadow-[inset_0_1px_0_rgba(255,255,255,0.04)]">
      <div className="mb-4 flex items-center gap-2">
        <span className={cn('h-2.5 w-2.5 rounded-full', connected ? 'bg-safe animate-pulse' : 'bg-destructive')} />
      ...
      {!HAS_CONFIGURED_BACKEND && (
        <p className="py-4 text-center text-sm text-yellow-400">Backend URL missing. Configure VITE_BACKEND_URL to enable live feed.</p>
      )}

      {HAS_CONFIGURED_BACKEND && events.length === 0 && (
        <p className="py-8 text-center text-sm text-slate-400">Waiting for scans...</p>
      )}
          ...
          {visibleEvents.map((event, index) => {
            ...
            return (
              <motion.div
                ...
                className="grid grid-cols-[96px_48px_minmax(0,1fr)] items-center gap-3 rounded-xl border border-white/10 bg-white/3 px-3 py-2.5 text-sm sm:grid-cols-[104px_56px_minmax(0,1fr)_52px_64px]"
              >
                <span className={cn('inline-flex w-fit rounded-full border px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide', verdictTone)}>{verdictText}</span>
                ...
                <span className="hidden text-right text-[11px] text-slate-500 sm:block">{event.timestamp ? formatTime(event.timestamp) : '--:--'}</span>
              </motion.div>
            );
          })}
        </AnimatePresence>
...
export default function Dashboard() {
  ...
  const enhanceWithPythonBackend = async (baseResult: DashboardResult | null, rawEmailText: string, rawHeaders: string): Promise<DashboardResult | null> => {
    ...
    const finalRiskScore = Math.max(0, Math.min(100, Math.round(Number(scanData.risk_score ?? baseResult?.riskScore ?? 0))));
    const finalClassification = normalizeClassification(
      scanData.classification
      ?? scanData.verdict
      ?? scoreToVerdictState(finalRiskScore),
    );
  ...
  const cardHoverClass = 'transition-all duration-200 ease-out hover:-translate-y-px hover:shadow-[0_8px_18px_rgba(2,6,23,0.14)]';
                    ...
                    {verdictColors && (
                      <div className="glass-panel relative overflow-hidden rounded-[28px] border border-[#D4AF37]/20 shadow-[0_24px_80px_rgba(2,6,23,0.32)]">
                        ...
                        <div className="relative z-10 flex flex-col gap-6 p-6 pl-8 sm:p-8">
                          ...
                          {displayClassification !== 'safe' && detectedSignals.length > 0 && (
                    ...
                    <div key={label} className={cn('rounded-2xl border shadow-[0_10px_24px_rgba(2,6,23,0.14)]', size === 'primary' ? 'p-5' : 'p-4', tone, cardHoverClass)}>
                      <div className="flex items-start justify-between gap-3">
                        <div>
                          ...
                          <p className={cn('mt-2 font-mono font-bold', size === 'primary' ? 'text-3xl' : 'text-2xl', color)}>{value}</p>
                        </div>
              ...
              <section className={cn(dashboardSectionClass, 'space-y-5')}>
                ...
                <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
                  {[
                    ...
                    <div key={card.label} className={cn('rounded-xl border border-card-border bg-card p-4', cardHoverClass)}>
